...
class DbalSessionHandler implements \SessionHandlerInterface
{
    ...
    {
        try {
            $this->con->executeQuery("DELETE FROM {$this->tableName} WHERE id = :id", array(
                'id' => $id,
            ));
    ...
    {
        try {
            $data = $this->con->executeQuery("SELECT data FROM {$this->tableName} WHERE id = :id", array(
                'id' => $id,
            ))->fetchColumn();
    ...
    public function write($id, $data)
    {
        ...
        if ($platform instanceof MySqlPlatform) {
            $sql = "INSERT INTO {$this->tableName} (id, data, sess_time) VALUES (%1\$s, %2\$s, %3\$d) "
